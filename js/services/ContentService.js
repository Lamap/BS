function ContentService(){
	...
	function onGetStructure(data){
		ConfigModel.tracer.log(data);	
		
		if(!data.status)
		{
			BSEvents.dispatchEvent(BSEvents.JSON_PARSE_ERROR,data);
			return;
		}
		
		if(data.status == "OK")		
			{
				ConfigModel.structureObjectTree = parseCategoryObjectToTree(data.structure);
				ConfigModel.structureObject = parseCategoryObjectToPlain(ConfigModel.structureObjectTree);
				ConfigModel.structureObject = repairTextContents(ConfigModel.structureObject);
				
				ConfigModel.mainMenu = ConfigModel.structureObjectTree[0].categories;
				ConfigModel.subMenu = ConfigModel.structureObjectTree[1].categories
				
				ConfigModel.mainPictures = ConfigModel.structureObjectTree[2].IMAGES;
				
				ConfigModel.fillPageIDs();
				
				BSEvents.dispatchEvent(BSEvents.STRUCTURE_LOADED,data);
			}					
		else
			BSEvents.dispatchEvent(BSEvents.STRUCTURE_LOAD_ERROR,data);
	};
